<template>
    <v-card>
        <v-toolbar dense color="accent">
            <v-btn icon @click="toggleDialog('suppliers')">
                <v-icon>close</v-icon>
            </v-btn>
        ...
        <v-card-text>
            <v-data-table
                ...
                <template slot="items" slot-scope="props">
                    ...
                    <td class="px-1">
                        <v-text-field
                                v-model="props.item.display_name"
                                hide-details
                        ></v-text-field>
                    </td>
                    <td class="px-1">
...
<script>
    ...
    export default {
        data: () => ({
            ...
            rowsPerPageItems: [7, 15, 30, 50],
            headers: [
                {text: 'Supplier name', value: 'name', sortable: false, width: '300'},
                {text: 'Display name', value: 'name', sortable: false, width: '300'},
        ...
        methods: {
            ...
            accountChange(supplier, $event) {
                if (typeof $event === 'string') {
                    supplier.name = $event
                }
                if (typeof $event === 'object') {
                    supplier.name = $event.name
                    supplier.display_name = $event.name
                    supplier.country_code = $event.country_code
                    supplier.data = $event.data
                    supplier.vat_no = $event.vat_no
                }
            },
            setItems() {
                while (this.items.length < this.pagination.rowsPerPage) {
                    this.items.push(JSON.parse(JSON.stringify(this.supplierStructure)))
                }
            },
            resetForm() {
                this.items = []
            ...
            save() {
                ...
                inventoryService.createSuppliers({items})
                    .then(() => {
                        this.toggleDialog('suppliers')
                    })
            },
            saveAsDraft() {
                draftService.createAsDraft(this.items, 'suppliers')
            },
            getLatestRecords() {
                balanceService.getLatestRecords('suppliers')
                    .then((response) => {
                        if(!response.length) {
            ...
            getDrafts() {
                draftService.getDraft('suppliers')
                    .then((response) => {
                        if(response.data) {
            ...
            isValidateTotalAmounts() {
                ...
                if(!this.tradePayablesAccount.transaction) {
                    return true
                }

                let accountAmount = this.tradePayablesAccount.transaction.credit || this.tradePayablesAccount.transaction.debit

                if(this.tradePayablesAccount.transaction.debit) {
