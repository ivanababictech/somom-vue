<template>
    <v-card>
        <accounting-toolbar
                ...
                account="empty"
                @toggleDialog="toggleDialog('journal_entries')"
        ...
        <v-btn @click="save" color="primary" class="ml-0" :disabled="Boolean(!validEntries.length || disableSave)">Save</v-btn>
    </v-card>
</template>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            saveAsDraft(){
                draftService.createAsDraft(this.entries, 'journal-entries')
            },
            init() {
        ...
        created() {
            draftService.getDraft('journal-entries')
                .then((response) => {
                    if(response.data) {
