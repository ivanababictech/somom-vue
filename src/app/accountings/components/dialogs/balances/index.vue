<template>
    <v-card>
        <v-toolbar dense color="accent">
            <v-btn icon @click="toggleDialog('balances')">
                <v-icon>close</v-icon>
            </v-btn>
        ...
        <v-card-text  class="mt-5">
            <date-picker label="Date" :value="firstLockPeriod.start_date" icon="event" :disabled="true" readonly></date-picker>
        ...
        methods: {
            ...
            save() {
                ...
                balancesService.create({accounts})
                    .then(response => {
                        this.resetForm()
                        this.$bus.$emit('load-opening-balances')
                    })
            },


            saveAsDraft() {
                draftService.createAsDraft(this.items, 'opening-balance')
            },
