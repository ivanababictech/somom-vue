...
<script>
    export default{
        ...
        computed: {
            totalBalance () {
                return this.creditors.reduce((total, debtor) => total += debtor.balance_at_year_end ,0)
            },
            totalPayables () {
                return this.creditors.reduce((total, debtor) => total += debtor.after_year_end_payables ,0)
            },
        }
    }
</script>